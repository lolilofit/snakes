...
object Resender {
    data class ResendObj(var ip: String, var port: Int, val message: SnakesProto.GameMessage, val timeSended: Long, val recverId: Int)
    ...
    private fun gamerDied(resendThis: ResendObj) {
        synchronized(ImmediateQueue::class) {
            ...
            if (SelfInfo.selfId != SelfInfo.masterInfo) {
                    resendThis.ip = master.ip_address
                    resendThis.port = master.port
            }
/*
        ...
        fun resendMessage(resendThis: ResendObj, nodeTimeoutMs: Int, socket: DatagramSocket) {
